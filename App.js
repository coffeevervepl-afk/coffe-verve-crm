          ...
          <div className="card">
            ...
            {byLang.length === 0 ? <div className="empty-state">{t.no_data}</div> : (
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {byLang.map(([l, cnt]) => (
                  <div key={l} style={{ textAlign: "center", background: "#1E1E1E", padding: "12px 16px", borderRadius: 8, flex: 1 }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "#C9A84C" }}>{cnt}</div>
          ...
          <div className="card">
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {ordersByStatus.map(([st, cnt]) => (
                <div key={st} style={{ background: "#1E1E1E", padding: "10px 14px", borderRadius: 8, display: "flex", alignItems: "center", gap: 8 }}>
                  <span className="status-dot" style={{ background: STATUS_COLORS[st] }} />
                  <span style={{ fontSize: 13 }}>{t[STATUS_KEYS[st]]}</span>
                  <span style={{ fontWeight: 700, color: "#C9A84C" }}>{cnt}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <tbody>
                  {recentOrders.map(o => (
                    <tr key={o.id}>
                      ...
                      <td style={{ color: "#888" }}>{o.products?.name || "—"}</td>
                      <td style={{ color: "#C9A84C" }}>{fmtMoney(o.total)}</td>
                      <td style={{ color: "#666" }}>{fmtDate(o.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={c.id} className="client-row" onClick={() => onSelect(c)}>
                    <td><div style={{ fontWeight: 500 }}>{c.name}</div><div style={{ fontSize: 11, color: "#666" }}>{c.client_code}</div></td>
                    <td style={{ color: "#888" }}>{c.username ? `@${c.username}` : c.telegram_id || "—"}</td>
                    <td><span className="badge" style={{ background: "#2A2A2A" }}>{c.language || "—"}</span></td>
                    <td style={{ color: "#888", fontSize: 12 }}>{c.source || "—"}</td>
                    <td style={{ color: "#888", fontSize: 12 }}>{c.city || "—"}</td>
                    <td style={{ color: "#666", fontSize: 12 }}>{fmtDate(c.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div>
      <div className="topbar">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 12, color: "#666" }}>{client.client_code}</span>
        </div>
        ...
        <div className="grid-2">
          ...
          <div className="card">
            ...
            <div className="client-detail">
              {[
                [t.name, client.name],
                ["Telegram", client.username ? `@${client.username}` : client.telegram_id || "—"],
                [t.phone, client.phone || "—"],
                [t.email, client.email || "—"],
                [t.language, client.language || "—"],
                [t.source, client.source || "—"],
                [t.city, client.city || "—"],
                [t.date, fmtDate(client.created_at)],
              ...
              {client.notes && <div style={{ marginTop: 8, padding: 10, background: "#1E1E1E", borderRadius: 6, fontSize: 13, color: "#888" }}>{client.notes}</div>}
            </div>
          </div>
                ...
                <tbody>
                  {orders.map((o, i) => (
                    <tr key={o.id} style={{ cursor: "pointer" }} onClick={() => setSelectedOrder(o)}>
                      <td style={{ color: "#666", fontSize: 12 }}>{orders.length - i}</td>
                      <td style={{ fontWeight: 500 }}>{o.products?.name || "—"}</td>
                      <td style={{ color: "#888" }}>{o.weight}г</td>
                      <td style={{ color: "#C9A84C" }}>{fmtMoney(o.total)}</td>
                      <td><span className="badge" style={{ background: STATUS_COLORS[o.status] + "33", color: STATUS_COLORS[o.status] }}>{t[STATUS_KEYS[o.status]]}</span></td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowOrderModal(false)}>
          <div className="modal">
            ...
            {orderForm.product_id && (() => {
              ...
              return price ? <div style={{ padding: "10px", background: "#1E1E1E", borderRadius: 6, marginBottom: 14, color: "#C9A84C", fontSize: 14 }}>{t.price}: {fmtMoney(price)}</div> : null;
            })()}
          ...
          <div className="qr-container" style={{ minWidth: 160 }}>
            {qrUrl ? (
              <>
                ...
                <div style={{ fontSize: 10, color: "#666", margin: "8px 0", fontFamily: "monospace" }}>{order.qr_token}</div>
                <button className="btn btn-secondary btn-sm" onClick={downloadQR}>{t.print_qr}</button>
              </>
            ) : <div style={{ color: "#666", fontSize: 12 }}>Генерация QR...</div>}
          </div>
        </div>
        <div className="modal-actions">
            ...
            <table className="table">
              ...
              <tbody>
                {filtered.length === 0 ? <tr><td colSpan={8} className="empty-state">{t.no_data}</td></tr> : filtered.map((o, i) => (
                  <tr key={o.id} style={{ cursor: "pointer" }} onClick={() => setSelectedOrder(o)}>
                    <td style={{ color: "#666", fontSize: 12 }}>{filtered.length - i}</td>
                    <td><div style={{ fontWeight: 500 }}>{o.clients?.name || "—"}</div><div style={{ fontSize: 11, color: "#666" }}>{o.clients?.client_code}</div></td>
                    <td style={{ color: "#888" }}>{o.products?.name || "—"}</td>
                    <td style={{ color: "#888" }}>{o.weight}г</td>
                    <td style={{ color: "#C9A84C", fontWeight: 600 }}>{fmtMoney(o.total)}</td>
                    ...
                    <td style={{ color: "#666", fontSize: 12 }}>{fmtDate(o.created_at)}</td>
                    <td><button className="btn btn-secondary btn-sm" onClick={e => { e.stopPropagation(); setSelectedOrder(o); }}>QR</button></td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <table className="table">
              ...
              <tbody>
                {products.map(p => (
                  <tr key={p.id}>
                    <td><div style={{ fontWeight: 500 }}>{p.name}</div><div style={{ fontSize: 11, color: "#666" }}>{p.code}</div></td>
                    <td style={{ color: "#888", fontSize: 12 }}>{p.country}</td>
                    <td style={{ color: "#888", fontSize: 12, maxWidth: 200 }}>{p.flavor_notes}</td>
                    <td style={{ color: "#C9A84C" }}>{p.price_250 ? `${p.price_250} zł` : "—"}</td>
              ...
              <tbody>
                {warranties.map(w => (
                  <tr key={w.id}>
                    ...
                    <td style={{ color: "#888" }}>{w.orders?.products?.name || "—"}</td>
                    <td style={{ color: "#888", fontSize: 12, maxWidth: 200 }}>{w.reason}</td>
                    <td>
                    ...
                    <td style={{ color: "#666", fontSize: 12 }}>{fmtDate(w.activated_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {staff.length === 0 ? <tr><td colSpan={5} className="empty-state">{t.no_data}</td></tr> : staff.map(s => (
                  <tr key={s.id}>
                    ...
                    <td><span className="badge" style={{ background: s.role === "owner" ? "#C9A84C22" : "#2A2A2A", color: s.role === "owner" ? "#C9A84C" : "#888" }}>{s.role === "owner" ? t.owner : t.employee}</span></td>
                    <td style={{ color: "#666", fontSize: 12 }}>{fmtDate(s.created_at)}</td>
